...
class [[nodiscard]] RouteLine2 final {
  ...
  public:
    ...
    [[using gnu: pure, always_inline]] [[nodiscard]]
    std::vector<::boyle::math::Vec2<T>> tangent(const std::vector<T>& s) const noexcept {
        return curve_.tangent(s);
    }

    [[using gnu: pure, always_inline]] [[nodiscard]]
    std::vector<::boyle::math::Vec2<T>> orthonormal(const std::vector<T>& s) const noexcept {
        return curve_.orthonormal(s);
    }
  ...
  private:
    [[using gnu: always_inline]] explicit RouteLine2(
        ::boyle::math::PiecewiseQuinticCurve2<T> curve
    ) noexcept
