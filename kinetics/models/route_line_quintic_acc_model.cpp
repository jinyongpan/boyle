...
#include "kinetics/models/route_line_quintic_acc_model.h"
...
std::array<double, 6> RouteLineQuinticAccModel::prorationCoeffs(
...
const std::size_t& RouteLineQuinticAccModel::num_samples() const noexcept { return num_samples_; }

const ::boyle::math::QpProblem<double, int>& RouteLineQuinticAccModel::qp_problem(
) const noexcept {
    return qp_problem_;
}

const ::boyle::math::OsqpSettings& RouteLineQuinticAccModel::settings() const noexcept {
    return settings_;
}

void RouteLineQuinticAccModel::setIntegrationRelation() noexcept {
    for (std::size_t i{1}; i < num_samples_ - 1; ++i) {
        ...
        qp_problem_.updateConstrainTerm(
            num_samples_ * 3 + i, constrain_vec_1, -boyle::math::kEpsilon,
            ::boyle::math::kEpsilon
        );
        qp_problem_.updateConstrainTerm(
            num_samples_ * 4 + i, constrain_vec_2, -boyle::math::kEpsilon,
            ::boyle::math::kEpsilon
        );
    }
    return;
...
void RouteLineQuinticAccModel::setSoftFences(const std::vector<SoftFence1d>& soft_fences) noexcept {
...
void RouteLineQuinticAccModel::setHardFences(const std::vector<HardFence1d>& hard_fences) noexcept {
...
void RouteLineQuinticAccModel::setVelocityRange(double lower_bound, double upper_bound) noexcept {
    for (std::size_t i{1}; i < num_samples_ - 1; ++i) {
...
void RouteLineQuinticAccModel::setAccelRange(double lower_bound, double upper_bound) noexcept {
    for (std::size_t i{1}; i < num_samples_ - 1; ++i) {
...
void RouteLineQuinticAccModel::setInitialState(
    double s0, double v0, double a0, double j0
) noexcept {
    if (!std::isnan(s0)) {
        qp_problem_.updateConstrainTerm(
...
void RouteLineQuinticAccModel::setFinalState(double sf, double vf, double af, double jf) noexcept {
    if (!std::isnan(sf)) {
        qp_problem_.updateConstrainTerm(
...
void RouteLineQuinticAccModel::setVelocityCost(
...
void RouteLineQuinticAccModel::setAccelCost(double accel_weight) noexcept {
    for (std::size_t i{0}; i < num_samples_ - 1; ++i) {
...
void RouteLineQuinticAccModel::setJerkCost(double jerk_weight) noexcept {
    for (std::size_t i{0}; i < num_samples_ - 1; ++i) {
...
void RouteLineQuinticAccModel::setSnapCost(double snap_weight) noexcept {
    for (std::size_t i{0}; i < num_samples_ - 1; ++i) {
...
RouteLineQuinticAccModel::Result RouteLineQuinticAccModel::solve() const noexcept {
...
void RouteLineQuinticAccModel::clear() noexcept {
    num_samples_ = 0;
    time_scale_ = 0.0;
    qp_problem_.clear();
    sample_ts_.clear();
    ...
    reciprocal_h3s_.clear();
    reciprocal_h4s_.clear();
}

std::size_t RouteLineQuinticAccModel::sIndex(std::size_t t_index) const noexcept { return t_index; }

std::size_t RouteLineQuinticAccModel::vIndex(std::size_t t_index) const noexcept {
    return num_samples_ + t_index;
}

std::size_t RouteLineQuinticAccModel::aIndex(std::size_t t_index) const noexcept {
