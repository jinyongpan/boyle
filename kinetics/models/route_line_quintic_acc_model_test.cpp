...
TEST_CASE("CostTest") {
    ...
    state_vec.reserve(num_samples * 4);
...
TEST_CASE("HeadwayScene") {
    ...
    SUBCASE("Yield") {
        ...
        std::vector<SoftFence1d> soft_fences{
            SoftFence1d(0, ::boyle::common::Actio::BLOCKING, bound_line, 10.0, 10.0)};
        route_line_acc_model.setSoftFences(soft_fences);
        motion = route_line_acc_model.solve().motion;
    }
    ...
    SUBCASE("Overtake") {
        ...
        std::vector<SoftFence1d> soft_fences{
            SoftFence1d(0, ::boyle::common::Actio::PUSHING, bound_line, 10.0, 10.0)};
        route_line_acc_model.setSoftFences(soft_fences);
        motion = route_line_acc_model.solve().motion;
    }
    ...
    if (plot_graph) {
        ...
        matplot::figure_handle fig = figure();
        fig->size(1600, 1000);
        std::vector<axes_handle> ax;
        for (std::size_t i = 0; i < 5; ++i) {
            ax.emplace_back(subplot(2, 3, i));
