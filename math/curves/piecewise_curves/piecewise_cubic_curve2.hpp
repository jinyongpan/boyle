...
class [[nodiscard]] PiecewiseCubicCurve2 final : public Curve2<PiecewiseCubicCurve2<T>, T> {
  ...
  public:
    ...
    [[using gnu: flatten, leaf]] explicit PiecewiseCubicCurve2(
        ...
        PiecewiseCubicFunction1<Vec2<T>, T> temp_vec2_of_s{temp_arc_lengths, anchor_points, b0, bf};
        ...
        vec2_of_s_ = PiecewiseCubicFunction1<Vec2<T>, T>{
    ...
    T curvature(T s) const noexcept {
        ...
        const std::vector<Vec2<T>>& ddys = vec2_of_s_.ddys();
        const std::size_t size = arc_lengths.size();
        const std::size_t pos =
            closetUpperElement(arc_lengths.cbegin(), arc_lengths.cend(), s) - arc_lengths.cbegin();
        if (pos == 0) {
            spdlog::warn(
            ...
            const Vec2<T> derivative = cuberpd(
    ...
    SlDuplet<T> inverse(Vec2<T> point) const noexcept {
        const std::vector<T>& arc_lengths = arcLengths();
        const std::vector<Vec2<T>>& anchor_points = anchorPoints();
        const std::vector<Vec2<T>>& ddys = vec2_of_s_.ddys();
        const std::size_t pos =
    ...
    [[using gnu: pure]]
    SlDuplet<T> inverse(Vec2<T> point, T start_s, T end_s) const noexcept {
        if (start_s > end_s) {
            spdlog::warn(
        ...
        const std::size_t istart =
            closetUpperElement(arc_lengths.cbegin(), arc_lengths.cend(), start_s) -
            arc_lengths.cbegin() - 1;
        const std::size_t iend =
            closetUpperElement(arc_lengths.cbegin(), arc_lengths.cend(), end_s) -
            arc_lengths.cbegin();
        const std::size_t pos =
            closetUpperElement(
    ...
    [[using gnu: pure, always_inline]]
    const std::vector<Vec2<T>>& anchorPoints() const noexcept {
  ...
  private:
    ...
    PiecewiseCubicFunction1<Vec2<T>, T> vec2_of_s_;
};
